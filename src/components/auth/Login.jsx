...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const resp = await fetch(`${process.env.API_URL}/api/auth/login`, {
        method: "POST",
        body: JSON.stringify({
          email, password
        }),
      ...
      const jwtInfo = jwtDecode(userInfo.token)
      ...
      if (userInfo) {
        ...
        navigate(from !== "/" ? from : (userInfo.role === "admin" ? "/admin/dashboard" : "/user/dashboard"));
      } else {
        // Invalid credentials
