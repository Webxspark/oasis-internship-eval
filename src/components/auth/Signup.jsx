...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const storedUsers = await fetch(`${process.env.API_URL}/admin/users`).then(res => res.json())
      ...
      // Add to stored users
      const signupReq = await fetch(`${process.env.API_URL}/api/auth/register`, {
        method: "POST",
      ...
      if (signupReq.status != 201) {
        const err = await signupReq.json()
        setError(err.message)
      }
      const newUserInfo = await signupReq.json()

      // Generate authentication token
      const mockToken = newUserInfo.token;
